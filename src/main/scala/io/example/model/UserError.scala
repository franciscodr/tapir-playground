...
case class PersonaNonGrata(name: String) extends UserError(errorMessage = "This user is not welcomed here")
...
case class UserIdNotValid(id: Long) extends UserError(errorMessage = s"The user id is not valid. It should be a positive number.")
...
case class UserHandleNotValid(handle: String) extends UserError(errorMessage = s"The user handle is not valid. It shouldn't start with number")
